...
- (void)viewDidLoad {
    ...
    preferences.animating.dismissDuration = PIPE_TEST_TOOLTIP_DURATION;
    ...
    tooltip.text = PIPE_TEST_TOOLTIP_TEXT;
...
- (IBAction)createClicked:(id)sender {
    ...
    NSString *videoCodec = @"mpeg4";

    NSLog(@"Testing PIPE with \'%@\' codec\n", videoCodec);

    NSString* ffmpegCommand = [PipeViewController generateCreateVideoWithPipesScript:image1:image2:image3:videoFile:videoCodec:@""];
...
- (void)showTooltip {
    [tooltip showAnimated:YES forView:self.createButton withinSuperView:self.view];
}
