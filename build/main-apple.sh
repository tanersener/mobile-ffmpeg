...
if [[ -z ${IPHONEOS_DEPLOYMENT_TARGET} ]]; then
    echo -e "(*) IPHONEOS_DEPLOYMENT_TARGET not defined\n"
    exit 1
fi
    ...
    do
        ...
        if [ $run -eq 1 ] && [[ -z ${!BUILD_COMPLETED_FLAG} ]]; then
            ...
            if [[ ${LIBRARY_IS_INSTALLED} -ne 0 ]] || [[ ${!REBUILD_FLAG} -eq 1 ]] || [[ ${!DEPENDENCY_REBUILT_FLAG} -eq 1 ]]; then
                ...
                SCRIPT_PATH="${BASEDIR}/build/apple-${library}.sh"
...
FFMPEG_INSTALLED=$(library_is_installed ${INSTALL_BASE} ffmpeg)
if [[ $(library_is_installed ${INSTALL_BASE} ffmpeg) -ne 0 ]] || [[ ${BUILD_FFMPEG} -eq 1 ]]; then

    # BUILDING FFMPEG
    . ${BASEDIR}/build/apple-ffmpeg.sh "$@" || exit 1
else
    echo -e "\nffmpeg: skipped"
fi
