...
get_library_name() {
    case $1 in
        ...
        43) echo "apple-zlib" ;;
        44) echo "apple-audiotoolbox" ;;
        45) echo "apple-bzip2" ;;
        46) echo "apple-videotoolbox" ;;
        47) echo "apple-avfoundation" ;;
        48) echo "apple-libiconv" ;;
        49) echo "apple-libuuid" ;;
    esac
}
...
get_arch_name() {
    case $1 in
        ...
        2) echo "arm64" ;;
        3) echo "arm64e" ;;
        4) echo "i386-simulator" ;;
        5) echo "x86-64-simulator" ;;
        6) echo "x86-64-mac-catalyst" ;;
        7) echo "arm64-mac-catalyst" ;;
        8) echo "x86-64-macos" ;;
        9) echo "arm64-macos" ;;
        10) echo "arm64-simulator" ;;
    esac
}
...
get_target_host() {
    case ${ARCH} in
        *-mac-catalyst)
            echo "$(get_arch_code)-apple-ios$(get_sdk_min_version)-macabi"
            ;;
        *-macos)
            echo "$(get_arch_code)-apple-macos$(get_sdk_min_version)"
            ;;
        *-simulator)
            echo "$(get_arch_code)-apple-ios$(get_sdk_min_version)-simulator"
            ;;
        *)
            echo "$(get_arch_code)-apple-ios$(get_sdk_min_version)"
            ;;
    esac
}
...
get_target_build_directory() {
    echo "$(get_platform_name)/$(get_arch_code)"
}
...
get_min_version_cflags() {
    case ${ARCH} in
        *-simulator)
            echo "-mios-simulator-version-min=$(get_sdk_min_version)"
            ;;
        *-mac-catalyst)
            echo "-miphoneos-version-min=$(get_sdk_min_version)"	
            ;;
        *-macos)
            echo "-mmacosx-version-min=$(get_sdk_min_version)"
            ;;
        *)
            echo "-miphoneos-version-min=$(get_sdk_min_version)"	
            ;;
...
get_common_cflags() {
    ...
    case ${ARCH} in
        *-simulator)
            ...
            ;;
        *-mac-catalyst)
            echo "-fstrict-aliasing ${LTS_BUILD_FLAG}${BUILD_DATE} -isysroot ${SDK_PATH}"
            ;;
        *-macos)
            echo "${LTS_BUILD_FLAG}${BUILD_DATE} -isysroot ${SDK_PATH}"
            ;;
        *)
...
get_cpu_flags() {
    case ${ARCH} in
        armv7)
            echo "-march=armv7 -mfpu=cortex-a8 -mfpu=neon -mfloat-abi=softfp -DMOBILE_FFMPEG_ARMV7"
            ;;
        armv7s)
            echo "-march=armv7s -mfpu=generic -mfpu=neon -mfloat-abi=softfp -DMOBILE_FFMPEG_ARMV7S"
            ;;
        arm64e)
            echo "-march=armv8.3-a+crc+crypto -mcpu=generic -DMOBILE_FFMPEG_ARM64E"
            ;;
        arm64*)
            echo " -march=armv8-a+crc+crypto -mcpu=generic -DMOBILE_FFMPEG_ARM64"
            ;;
        x86-64-*)
            echo "-march=x86-64 -msse4.2 -mpopcnt -m64 -mtune=intel -DMOBILE_FFMPEG_X86_64"
            ;;
        i386-*)
            echo "-march=i386 -mtune=intel -mssse3 -mfpmath=sse -m32 -DMOBILE_FFMPEG_I386"
            ;;
    esac
}
...
get_arch_specific_cflags() {
    case ${ARCH} in
        *-mac-catalyst)
            echo "-arch $(get_arch_code)  -target $(get_target_host) $(get_cpu_flags) -isysroot ${SDK_PATH} -isystem ${SDK_PATH}/System/iOSSupport/usr/include -iframework ${SDK_PATH}/System/iOSSupport/System/Library/Frameworks -F${SDK_PATH}/System/iOSSupport/System/Library/Frameworks"
            ;;
        *)
            echo "-arch $(get_arch_code) -target $(get_target_host) $(get_cpu_flags)"
            ;;
    esac
}
...
get_size_optimization_asm_cflags() {
    ...
    case $1 in
        jpeg | ffmpeg)
            ...
            ;;
        *)
            ARCH_OPTIMIZATION=$(get_size_optimization_cflags)
            ;;
    esac
...
get_app_specific_cflags() {
    ...
    case $1 in
        ...
        sdl2)
            case ${ARCH} in
                *-macos)
                APP_FLAGS="-DPIC -Wno-unused-function"
                ;;
                *)
...
get_cflags() {
    ...
    if [[ -z ${MOBILE_FFMPEG_DEBUG} ]]; then
        local OPTIMIZATION_FLAGS=$(get_size_optimization_cflags)
    else
        local OPTIMIZATION_FLAGS="${MOBILE_FFMPEG_DEBUG}"
    fi
    local MIN_VERSION_FLAGS=$(get_min_version_cflags)
...
get_cxxflags() {
    local COMMON_CFLAGS="$(get_common_cflags $1) $(get_common_includes $1) $(get_arch_specific_cflags) $(get_min_version_cflags)"
    if [[ -z ${MOBILE_FFMPEG_DEBUG} ]]; then
    ...
    local BITCODE_FLAGS=""
    case ${ARCH} in
        armv7 | armv7s | arm64 | arm64e | arm64-mac-simulator)
...
get_arch_specific_ldflags() {
    case ${ARCH} in
        armv7)
            echo "-arch armv7 -march=armv7 -mcpu=neon -mfloat-abi=softfp -fembed-bitcode=all -target $(get_target_host)"
            ;;
        armv7s)
            echo "-arch armv7s -march=armv7s -mcpu=neon -mfloat-abi=softfp -fembed-bitcode=all -target $(get_target_host)"
            ;;
        arm64)
            echo "-arch arm64 -march=armv8-a+crc+crypto -fembed-bitcode=all -target $(get_target_host)"
            ;;
        arm64e)
            echo "-arch arm64e -march=armv8.3-a+crc+crypto -fembed-bitcode=all -target $(get_target_host)"
            ;;
        x86-64-mac-catalyst)
            echo "-arch x86_64 -march=x86-64 -target $(get_target_host) -isysroot ${SDK_PATH} -L${SDK_PATH}/System/iOSSupport/usr/lib -F${SDK_PATH}/System/iOSSupport/System/Library/Frameworks -iframework ${SDK_PATH}/System/iOSSupport/System/Library/Frameworks"
            ;;
        arm64-mac-catalyst)
            echo "-arch arm64 -march=armv8-a+crc+crypto -target $(get_target_host) -isysroot ${SDK_PATH} -L${SDK_PATH}/System/iOSSupport/usr/lib -F${SDK_PATH}/System/iOSSupport/System/Library/Frameworks -iframework ${SDK_PATH}/System/iOSSupport/System/Library/Frameworks" 
            ;;
        arm64-macos)
            echo "-arch arm64 -march=armv8-a+crc+crypto -target $(get_target_host)" 
            ;;    
        i386*)
