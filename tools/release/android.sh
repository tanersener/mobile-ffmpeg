...
gradle -b ${BASEDIR}/../../android/app/release.template.gradle -PreleaseVersionCode=$1 -PreleaseVersionName=$2 -PreleaseProject=mobile-ffmpeg-min -PreleaseProjectDescription='Includes FFmpeg v3.4.2 without any external libraries enabled.' clean install bintrayUpload || exit 1
create_package "min" "1.1" || exit 1
...
create_package "min-gpl" "1.1" || exit 1
...
create_package "https" "1.1" || exit 1
...
create_package "https-gpl" "1.1" || exit 1
...
create_package "full" "1.1" || exit 1
...
create_package "full-gpl" "1.1" || exit 1
