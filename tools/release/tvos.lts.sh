#!/bin/bash
# 
# Creates a new tvos lts release from current branch
#

export BASEDIR=$(pwd)
export SOURCE_PACKAGE="${BASEDIR}/../../prebuilt/tvos-framework"
...
export CUSTOM_OPTIONS="--lts --enable-tvos-zlib --enable-tvos-bzip2 --enable-tvos-coreimage --enable-tvos-audiotoolbox"
...
create_package() {
    ...
    zip -r "mobile-ffmpeg-$1-${PACKAGE_VERSION}-tvos-static-universal.zip" ${PACKAGE_NAME}-universal || exit 1
}
...
# MIN RELEASE
cd ${BASEDIR}/../.. || exit 1
./tvos.sh ${CUSTOM_OPTIONS} || exit 1
create_package "min" "$1.LTS" || exit 1

# MIN-GPL RELEASE
cd ${BASEDIR}/../.. || exit 1
./tvos.sh ${CUSTOM_OPTIONS} ${GPL_PACKAGES} || exit 1
create_package "min-gpl" "$1.LTS" || exit 1

# HTTPS RELEASE
cd ${BASEDIR}/../.. || exit 1
./tvos.sh ${CUSTOM_OPTIONS} --enable-gnutls --enable-gmp || exit 1
create_package "https" "$1.LTS" || exit 1

# HTTPS-GPL RELEASE
cd ${BASEDIR}/../.. || exit 1
./tvos.sh ${CUSTOM_OPTIONS} --enable-gnutls --enable-gmp ${GPL_PACKAGES} || exit 1
...
# FULL RELEASE
cd ${BASEDIR}/../.. || exit 1
./tvos.sh ${CUSTOM_OPTIONS} ${FULL_PACKAGES} || exit 1
create_package "full" "$1.LTS" || exit 1

# FULL-GPL RELEASE
cd ${BASEDIR}/../.. || exit 1
./tvos.sh ${CUSTOM_OPTIONS} ${FULL_PACKAGES} ${GPL_PACKAGES} || exit 1
